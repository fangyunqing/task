...
def rotate_by_factor(src: Tensor, angle_factor: float) -> Tensor:
    ...
    if angle_factor != 0:
        ...
        dst = functional.rotate(src, angle_deg, functional.InterpolationMode.BILINEAR)
    else:
        dst = src
...
def rotate_square(src: Tensor, angle_factor: float) -> Tensor:
    ...
    if div_factor != 1.0:  # sin(pi/2) is identical to 1.0
        ...
        dst = functional.center_crop(dst, crop_t)

    return dst


def square_resize(src: Tensor, target_size) -> Tensor:
    """
...
def from_img(src: Tensor, angle_factor: float, target_size) -> Tensor:
    """
...
def strip_border(src: Tensor) -> Tensor:
    ...
    dst = functional.center_crop(src, src_size / SQRT2)

    return dst


def from_captcha(src: Tensor, angle_factor: float, target_size) -> Tensor:
    """
